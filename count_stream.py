...
# ─────────────────────────────────────────────────────────────────────────────
# >>> STREAMING: FFmpeg publisher to MediaMTX (RTMP)
import subprocess

OUT_W, OUT_H = 1280, 720               # stream resolution (what viewers see)
STREAM_FPS   = 20                      # stream frame rate (15–20 is fine)
RTMP_URL     = "rtmp://127.0.0.1/live/stream"  # MediaMTX on same server
FFMPEG_BIN   = "ffmpeg"                # or r"C:\ffmpeg\bin\ffmpeg.exe"
...
ffmpeg = subprocess.Popen([
    ...
    "-g", str(STREAM_FPS*2),          # keyframe ~2s
    "-f","flv", RTMP_URL
], stdin=subprocess.PIPE)

def publish_frame(bgr_frame):
    """Send the annotated frame to MediaMTX/RTMP via ffmpeg."""
    if bgr_frame is None:
        return
    if bgr_frame.shape[1] != OUT_W or bgr_frame.shape[0] != OUT_H:
        bgr_frame = cv2.resize(bgr_frame, (OUT_W, OUT_H), interpolation=cv2.INTER_AREA)
    try:
        ffmpeg.stdin.write(bgr_frame.tobytes())
    except BrokenPipeError:
        pass  # ffmpeg died; you could respawn it here if needed
...
try:
    while True:
        ...
        if do_draw:
            ...
            cv2.line(frame, (X_MAX, LINE_Y-10), (X_MAX, LINE_Y+10), 1)
            ...
            if _key == 27:  # ESC key to quit
                break

            # >>> STREAMING: push the annotated frame to MediaMTX
            publish_frame(frame)
    ...
    stop_flag = True
    # tidy up ffmpeg stdin so process exits cleanly
    try:

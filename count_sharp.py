...
# ── Counting stability knobs (ADDED) ─────────────────────────────────────────
GATE_BAND = 12             # require crossing through a band around LINE_Y
COUNT_COOLDOWN_S = 0.20    # ignore re-counts too soon at nearly same x (debounce)
    ...
    while True:
        ...
        if do_infer:
            ...
            for cx, cy, ty in detections:
                ...
                # SAFER prev_ty: don't assume "from above" on first sighting (CHANGED)
                prev_ty = tracks.get(track_id, (cx, cy, ty))[2]

                # Banded crossing + debounce (CHANGED)
                crossed_down = (prev_ty < (LINE_Y - GATE_BAND)) and (ty >= (LINE_Y + GATE_BAND))

                if crossed_down and (track_id not in counted_ids):
                    t_now2 = time.time()
                    too_soon_same_place = (
                        (t_now2 - last_count_ts) < COUNT_COOLDOWN_S and
                        (last_count_cx is not None) and
                        (abs(cx - last_count_cx) < 40)
                    )

                    if not too_soon_same_place:
                        total_count += 1
                        counted_ids.add(track_id)
                        last_count_ts = t_now2
                        last_count_cx = cx
